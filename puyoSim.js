...
let currentEditColor = COLORS.EMPTY; // エディットモードで選択中の色 (デフォルトは消しごむ: 0)
...
function initializeGame() {
    ...
    // エディット用のネクストリストを初期化
    nextPuyoColors = JSON.parse(JSON.stringify(nextPuyoColors));
...
/**
 * ぷよの向きを判定する関数
 * @returns {string} 'vertical' (縦) または 'horizontal' (横)
 */
function getPuyoOrientation() {
    if (!currentPuyo) return 'vertical';
    
    // rotation 0, 2: 縦向き (Y方向に並んでいる)
    // rotation 1, 3: 横向き (X方向に並んでいる)
