/* puyoAI.js
 * Robust main-thread wrapper for the worker AI
 */
(function () {
    'use strict';
    ...
    function localEvaluateBoard(boardState) {
        const heights = Array(W).fill(0);
        for (let x = 0; x < W; x++) {
            for (let y = H - 1; y >= 0; y--) {
                if (boardState[y][x] !== C().EMPTY) {
        ...
        let s = -holes * 36 - bumpiness * 10 - maxH * 28;
        if (boardState[AI_CONFIG.DANGER_Y] && boardState[AI_CONFIG.DANGER_Y][AI_CONFIG.DANGER_X] !== C().EMPTY) s -= 500000;
        return s;
    }
    ...
    function localSolve(snapshot) {
        ...
        for (const p of placements) {
            const after = resolveBoard(placePiece(snapshot.board, current, p.x, p.y, p.rotation));
            let score = localEvaluateBoard(after.board) + after.chains * 30000 + after.score * 0.3 + after.attack * 2000;

            const next = snapshot.pieces[1];
            if (next) {
                const nextPlacements = placementsFor(after.board, next).slice(0, AI_CONFIG.LOCAL_BEAM_WIDTH);
                let childBest = -Infinity;
                for (const np of nextPlacements) {
                    const child = resolveBoard(placePiece(after.board, next, np.x, np.y, np.rotation));
                    const childScore = localEvaluateBoard(child.board) + child.chains * 30000 + child.score * 0.3 + child.attack * 2000;
                    if (childScore > childBest) childBest = childScore;
                }
                if (Number.isFinite(childBest)) score += childBest * 0.45;
            }
    ...
    function ensureWorker() {
        ...
        try {
            ...
            console.warn('module worker unavailable, AI will use local fallback:', err);
            worker = null;
            return null;
        }
        ...
        worker.onerror = (err) => {
            console.warn('AI worker error, falling back to local solver:', err);
            clearTimeout(timeoutId);
            timeoutId = null;
    ...
    function requestBestMove(snapshot) {
        const w = ensureWorker();
        if (!w) {
            return Promise.resolve(localSolve(snapshot));
        }
        ...
        return new Promise((resolve) => {
            ...
            timeoutId = setTimeout(() => {
                ...
                if (currentResolve) currentResolve(localSolve(snapshot));
    ...
    function moveLooksLegal(move) {
        const cur = safeCurrentPuyo();
        const b = safeBoard();
        if (!cur || !move || !b) return false;

        const test = { mainColor: cur.mainColor, subColor: cur.subColor };
        return canPlace(b, test, move.x, move.y, move.rotation);
    }
    ...
    async function doAI() {
        ...
        try {
            let move = await requestBestMove(snapshot);
            if (!moveLooksLegal(move)) {
                move = localSolve(snapshot);
            }
            ...
            const fallback = localSolve(snapshot);
